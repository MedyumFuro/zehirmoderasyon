...
client.on("guildMemberAdd", member => {  
  const kanal = member.guild.channels.cache.find(r => r.id === "806176973914374144");
  ...
  if (kurulus > 1296000000) kontrol = ''
  moment.locale("tr");
  kanal.send(":tada: Sunucumuza Hoş Geldin ! <@" + member + ">  \n\n <a:803741735850344479:806185352074625096>  Hesabın "+ gecen +" Önce Oluşturulmuş. "+kontrol+" \n\n Sunucu kurallarımız <#806176980843102268> kanalında belirtilmiştir, Unutma sunucu içerisinde ki ceza işlemlerin kuralları okuduğunu varsayarak gerçekleştirilecek. \n\n Seninle beraber " + member.guild.memberCount + " kişi olduk ! Tagımızı alarak bizlere destek olabilirsin ! Kayıt olmak için teyit odalarına girip ses teyit vermen gerekiyor <@&806176852469219378> yetkililerimiz seninle ilgilenecektir ! İyi eğlenceler.  ")
  });
...
client.on("guildMemberAdd", member => {
  member.roles.add('806176850534989825'); // UNREGİSTER ROLÜNÜN İDSİNİ GİRİN
});

//-----------------------BOT-SESE-SOKMA-------------------------\\
client.on("ready", () => {
  client.channels.cache.get("806176957522771988").join();   
}) 
